...
ab_set = set()
for A in a_set:
    for B in get_b_set(256*r0,lc,A):
...
for p in integer_d_set:
    p = R(p)
    (f,g) = get_sol(p)
    if any(c not in ZZ for c in f.coefficients()) or any(c not in ZZ for c in g.coefficients()):
        raise Exception("Bad Examples")
    print("d = {}".format(p))
    print("f = {}".format(f))
    print("g = {}".format(g))
    print()

# notice that the two solutions are actually linearly equivalent to one another over Z.

p = x^4 + 2*x^3 - 7*x^2 - 4*x + 10
q = x^4 + 2*x^3 - 7*x^2 - 12*x + 6
